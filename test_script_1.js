    ...
    function parseIndoDate(str) {
        ...
        // 1. If it's an ISO string from Google Sheets (e.g. 2026-07-16T17:00:00.000Z), 
    ...
    function renderDashboard() {
        ...
        // Card 4: Setoran Hari Ini (always today) (no ekstra)
        const setoranToday = todayData.reduce((s, l) => s + l.totalHarga, 0) - todayData.reduce((s, l) => s + l.totalKomisi, 0);
        document.getElementById('cardSetoranToday').textContent = formatRupiah(setoranToday);
    ...
    function renderTodayTable() {
        ...
        const elSetoran = document.getElementById('cardSetoranToday');
        ...
        sortedGroups.forEach((g, index) => {
            ...
            sortedInnerKeys.forEach(key => {
                ...
                ig.bookings.forEach(b => {
                    ...
                    const badgeText = isPaid ? 'LUNAS' : 'BELUM ACC';
    ...
    function updateNotifications() {
        ...
        const requests = allBookings.filter(b => b.status === 'waiting_payment' || b.status === 'Belum ACC');
    ...
    function renderArmada() {
        ...
        if (!armadas.length) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center text-sm text-slate-400 font-bold py-8 italic uppercase tracking-widest">Belum ada data armada</td></tr>';
            return;
        }
            ...
            groups[key].forEach(a => {
                html += `
                    <tr class="armada-group-${index} ${hiddenClass} hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
                        ...
                        <td><span class="font-black text-sm text-slate-400 uppercase hidden">${a.name}</span></td>
                        ...
                        <td class="text-center"><div class="flex gap-1 justify-center"><button onclick="openEditArmadaModal('${a.id}')" class="bg-amber-500 hover:bg-amber-600 text-white w-7 h-7 flex items-center justify-center rounded-lg text-xs shadow-sm transition-all mx-auto" title="Edit Armada"><i class="fas fa-edit"></i></button>
                        <button onclick="openDeleteArmadaModal('${a.id}')" class="bg-red-500 hover:bg-red-600 text-white w-7 h-7 flex items-center justify-center rounded-lg text-xs shadow-sm transition-all mx-auto" title="Hapus Armada"><i class="fas fa-trash"></i></button></div></td>
                    </tr>
                `;
            });
        });
    ...
    async function submitAdminBooking(e) {
        ...
        try {
            ...
            if(res.status === 'success') {
                ...
                if(document.getElementById('ticketTujuan')) document.getElementById('ticketTujuan').innerText = payload['Tujuan'];
    ...
    function updateAdminTime() {
        ...
        if (!selectedName) {
            ...
            const armadas = getArmadas();
            const filteredByName = armadas.filter(a => a.name === selectedName);
    ...
    function updateAdminRute() {
        ...
        if (!selectedName || !selectedTime) {
            ...
            const armadas = getArmadas();
            const filtered = armadas.filter(a => a.name === selectedName && a.time === selectedTime);
    ...
    function printRawBT() {
        ...
        addTwoColumnsLeftRight("Asal", "Kawunganten", true);
        addTwoColumnsLeftRight("Tujuan", t.tujuan, true);
