...
// EXPORT PESANAN
// ==========================================
function exportPesanan(format) {
    if (!filteredPesanan || filteredPesanan.length === 0) {
        showMessage("Tidak ada data untuk diexport", true);
        return;
    }

    if (format === 'csv') {
        const headers = ['ID_TIKET', 'TANGGAL PEMBERANGKATAN', 'NAMA', 'NOMOR HP', 'JENIS KENDARAAN', 'Tujuan', 'JUMLAH PNP', 'WAKTU', 'NOMOR KURSI', 'Total Harga', 'PEMBAYARAN', 'Status'];
        ...
        link.setAttribute('download', `Export_Pesanan_${new Date().getTime()}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    } else if (format === 'pdf') {
        if (typeof window.jspdf === 'undefined') {
            showMessage("Library PDF belum dimuat, silakan coba lagi.", true);
            return;
        }
        const { jsPDF } = window.jspdf;
        ...
        doc.setFontSize(14);
        doc.text("Daftar Pesanan", 14, 15);
        
        const tableBody = filteredPesanan.map(p => [
            p['ID_TIKET'], 
            p['TANGGAL PEMBERANGKATAN'],
            p['NAMA'],
            p['JENIS KENDARAAN'] + ' - ' + p['Tujuan'],
            p['JUMLAH PNP'],
            'Rp ' + parseInt(p['Total Harga'] || 0).toLocaleString('id-ID'),
            p['PEMBAYARAN']
        ]);
        
        doc.autoTable({
            head: [['ID', 'Tanggal', 'Nama', 'Armada', 'Pnp', 'Total', 'Bayar']],
            body: tableBody,
            startY: 20,
            styles: { fontSize: 8 }
        });
